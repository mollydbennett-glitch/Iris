...
export default function ItemDetailPage() {
  ...
  async function rotate(degrees) {
    ...
    try {
      ...
      // New unique URL; cutout was cleared server-side.
      setIt((prev) => ({ ...prev, image_url: data.image_url, cutout_url: null }));
    } catch (err) {
      setError(err.message);
    } finally {
      setRotating(false);
  ...
  const showCutout = !!it.cutout_url;
  const imgSrc = it.cutout_url || it.image_url;
