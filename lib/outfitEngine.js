...
GAP, when there is one: lead with a workaround the user can do TODAY from their own closet, then name the one specific missing piece and roughly how many more looks owning it would unlock. Only flag a gap when a look genuinely needs a piece they don't own. Never generic "buy more."
...
{ "outfits": [ {
  ...
  "gap": { "workaround": "what to do now with what they own", "suggested": "the one missing piece, or null", "impact": integer or null } or null
} ] }
...
Generate ${candidates} candidate outfit${candidates === 1 ? '' : 's'}, each fully scored. Make them genuinely different from one another. I will keep the strongest.`;
  ...
  const outfits = final.map((o) => {
    ...
    const gapText = g ? (g.workaround || g.suggested || null) : (typeof o.gap === 'string' ? o.gap : null);
    return {
