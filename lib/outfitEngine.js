...
- When "learned from their ratings" is provided, treat it as direct recent evidence of what they actually like: repeat what keeps getting a yes, avoid what keeps getting a no. It sits alongside their references and outranks generic rules.
...
- Styling guidance: ${stylingRulesText(signature?.styling_rules)}
- Learned from their yes and no ratings on past looks (recent, strong signal): ${signature?.learned_preferences || 'no rated looks yet'}${playbookText(signature?.proportion_playbook)}
...
- Styling guidance: ${stylingRulesText(signature?.styling_rules)}
- Learned from their yes and no ratings on past looks: ${signature?.learned_preferences || 'no rated looks yet'}${playbookText(signature?.proportion_playbook)}
