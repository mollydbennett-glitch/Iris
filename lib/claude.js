...
// Turns the user's recent yes/no ratings on generated looks into a short set
// of learned preferences the styling engine reads on every generation. This
// runs on the top model because it is judgment work: real recurring patterns
// only, grounded in the actual rated looks, never invented.
const LEARN_PROMPT = `You are a stylist's assistant studying which generated outfits a user said yes or no to.

You will receive a JSON list of rated looks. Each has a verdict ("yes" or "no"), the pieces in it, and sometimes the user's own note.

Write the patterns that actually recur, as short plain lines the stylist can act on. Cover, only where the data supports it:
- pieces, or kinds of pieces, that keep landing in yes looks
- combinations, colors, or proportions that keep landing in no looks
- anything the user's notes say directly (their own words outrank any inference)

Rules:
- Only patterns visible in the data. A single example is a signal, not a pattern; leave it out or say it is early.
- 8 lines maximum, plain text, no headers, no markdown, no dashes as punctuation.
- Never invent a preference. If the data is thin, return fewer lines. If there is nothing real to say, return exactly: not enough rated looks yet.`;

export async function summarizeRatedLooks(ratedLooks) {
  const msg = await anthropic.messages.create({
    model: 'claude-fable-5',
    max_tokens: 600,
    system: LEARN_PROMPT,
    messages: [{ role: 'user', content: JSON.stringify(ratedLooks) }],
  });
  const tb = msg.content.find((b) => b.type === 'text');
  return (tb ? tb.text : '').trim();
}
